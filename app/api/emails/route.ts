...
export async function POST(request: Request) {
  ...
  const { baseUrl, headers } = supabaseConfig();
  const select = "id,evaluation_score,old_salary,raise_percent,comment_1,comment_2,comment_3,comment_4,comment_5,cycle:evaluation_cycles(academic_year),employee:employees(full_name,email,position,national_id,bank_account,active)";
  ...
  const rows = ((await dataResponse.json()) as EvaluationRow[]).filter(row => row.employee?.active !== false);
  ...
  const invalid = orderedRows.find(row => row.evaluation_score === null || Number(row.old_salary) <= 0 || !emailPattern.test(row.employee.email));
  if (invalid) return NextResponse.json({ error: `ข้อมูลของ ${invalid.employee.full_name} ยังไม่พร้อมส่ง` }, { status: 409 });
  ...
  for (let index = 0; index < orderedRows.length; index += 1) {
    ...
    const recipient = isTest ? testRecipient : row.employee.email.trim().toLowerCase();
    ...
    if (delivery.status === "sent") {
      results.push({ evaluationId: row.id, name: row.employee.full_name, email: recipient, status: "skipped", sentAt: delivery.sent_at ?? undefined });
      continue;
    }
    ...
    try {
      ...
      const info = await transporter.sendMail({
        ...
        html: personnelEmailHtml(row.employee.full_name, row.cycle.academic_year),
        attachments: [{ filename: `หนังสือแจ้งผลประเมิน_${safeFileName(row.employee.full_name)}_ลับ.pdf`, content: Buffer.from(pdf), contentType: "application/pdf" }],
      });
      ...
      results.push({ evaluationId: row.id, name: row.employee.full_name, email: recipient, status: "sent", sentAt });
    } catch (error) {
      ...
      results.push({ evaluationId: row.id, name: row.employee.full_name, email: recipient, status: "failed", error: message });
    }
