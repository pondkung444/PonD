...
export default function Home() {
  ...
  async function downloadAllPdfs(){
    ...
    try{
      ...
      link.href=url;link.download="หนังสือแจ้งผลประเมิน_บุคลากรทั้งหมด_2569_ลับ.zip";document.body.appendChild(link);link.click();link.remove();window.setTimeout(()=>URL.revokeObjectURL(url),1_000);
  ...
  async function importExcel(file: File) {
    try {
      ...
      const previewResponse=await fetch("/api/imports",{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify({academicYear:2569,fileName:file.name,confirm:false,rows:serializeImportRows(imported)})});
  ...
  async function confirmImport(){
    ...
    const response=await fetch("/api/imports",{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify({academicYear:2569,fileName:pendingImport.fileName,confirm:true,rows:serializeImportRows(pendingImport.rows)})});
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="identity"><div className="logo">ป</div><div><strong>ระบบแจ้งผลประเมินบุคลากร</strong><span>โรงเรียน มอ. วิทยานุสรณ์ สุราษฎร์ธานี</span></div></div><div className="cycle">ปีการศึกษา 2569</div></header>
