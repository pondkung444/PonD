...
type ApiEvaluationRow = { id:string; evaluation_score:number|string|null; old_salary:number|string; raise_percent:number|string; comment_1:string; comment_2:string; comment_3:string; comment_4:string; comment_5:string; note:string; snapshot_full_name:string|null; snapshot_email:string|null; snapshot_position:string|null; snapshot_national_id:string|null; snapshot_bank_account:string|null; snapshot_personnel_group:string|null; snapshot_source_sheet:string|null; employee:{ id:string; employee_code:string|null; full_name:string; email:string; position:string; national_id:string; bank_account:string; active:boolean; personnel_group:string|null; source_sheet:string|null } };
...
const sample: Person[] = [
  { id:"EMP-001", status:"ปฏิบัติงาน", name:"นางสาวตัวอย่าง หนึ่ง", email:"person1@school.ac.th", position:"ครูประจำการวิชาคณิตศาสตร์", nationalId:"1-2345-67890-12-3", bankAccount:"123-4-56789-0", score:88.5, raisePercent:4, oldSalary:24200, comments:["มีความรับผิดชอบและพัฒนาการจัดการเรียนรู้อย่างต่อเนื่อง",""], note:"", source:"ตัวอย่าง" },
  { id:"EMP-002", status:"ปฏิบัติงาน", name:"นายตัวอย่าง สอง", email:"person2@school.ac.th", position:"ครูประจำการวิชาวิทยาศาสตร์", nationalId:"2-3456-78901-23-4", bankAccount:"234-5-67890-1", score:84, raisePercent:3.5, oldSalary:21800, comments:["ควรพัฒนาการจัดเก็บหลักฐานผลการปฏิบัติงานให้เป็นระบบ",""], note:"", source:"ตัวอย่าง" },
  { id:"EMP-003", status:"ปฏิบัติงาน", name:"นางตัวอย่าง สาม", email:"", position:"เจ้าหน้าที่บริหารงานทั่วไป", nationalId:"", bankAccount:"", score:null, raisePercent:0, oldSalary:19500, comments:[""], note:"", source:"ตัวอย่าง" },
];
...
export default function Home() {
  ...
  async function loadPeople(accessToken:string,year=academicYear){
    ...
    const loaded:Person[]=data.rows.map((row:ApiEvaluationRow)=>({
      ...
      oldSalary:Number(row.old_salary), comments:[row.comment_1,row.comment_2,row.comment_3,row.comment_4,row.comment_5], note:row.note||"",
  ...
  return <div className="app-shell">
    ...
    <main>
      {step==="import"&&<section className="page narrow"><div className="page-title"><div><h1>นำเข้าข้อมูลจาก Excel · ปี {academicYear}</h1><p>{cycleStatus==="closed"?"รอบนี้เป็นประวัติและถูกล็อกแล้ว เลือกรอบใหม่เพื่อทำข้อมูล":"ระบบจะเปรียบเทียบคนเดิม คนเข้าใหม่ คนพ้นสภาพ และข้อมูลที่เปลี่ยนก่อนบันทึก"}</p></div></div>
        ...
        <div className="safe-note"><strong>ข้อมูลที่ระบบใช้</strong><span>ชื่อ–สกุล · ตำแหน่ง · อีเมล · เลขบัตรประชาชน · บัญชีธนาคาร · ผลประเมิน · ร้อยละที่เพิ่ม · เงินเดือนเดิม · ข้อเสนอแนะ 1–5 และหมายเหตุ (ถ้ามี)</span></div>
      </section>}
