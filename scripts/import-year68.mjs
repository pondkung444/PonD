...
const filePath = "C:/Users/ASUS FX505/Documents/performance/outputs/year68-ready/แบบกรอกผลประเมิน_แยกตามกลุ่มบุคลากร(2).xlsx";
...
const rows=sourceRows.map((row,index)=>({
  ...
  raise_percent:numeric(pick(row,keys.raise)), comments:keys.comments.map(names=>clean(pick(row,names))), note:clean(pick(row,keys.note)),
}));
...
await rest("import_batches",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify([{cycle_id:cycle.id,file_name:"แบบกรอกผลประเมิน_แยกตามกลุ่มบุคลากร(2).xlsx",imported_rows:rows.length,rejected_rows:0}])});
