...
const cycles=await get("evaluation_cycles?select=id,academic_year&academic_year=eq.2569");
...
console.log(JSON.stringify({employees:employees.rows.length,cycles2569:cycles.rows.length,evaluations:evaluations.rows.length,duplicateEmailGroups,missingNationalId:employees.rows.filter(r=>!r.national_id).length,missingBankAccount:employees.rows.filter(r=>!r.bank_account).length}));
